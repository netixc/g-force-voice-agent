...
The Talker uses an OpenRouter API key from private `.env`. Pi uses a ChatGPT Codex OAuth credential stored in the private `PI_AGENT_DATA_VOLUME`; `pi-auth-init` copies only the `openai-codex` entry and never places it in the application image or workspace.
