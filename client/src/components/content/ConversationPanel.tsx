...
export function ConversationPanel() {
  ...
  const [textSendError, setTextSendError] = useState("");
  ...
  const handleTextSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!content || !client || isSendingText) return;
    ...
    try {
      ...
      injectMessage({
        role: "user",
        parts: [{ text: content, final: true, createdAt }],
      });
    } catch (error) {
      setTextInput(content);
      setTextSendError(error instanceof Error ? error.message : "Unable to send message");
    } finally {
      setIsSendingText(false);
    }
  };
  ...
  return (
    <div className="conversation-panel p-4">
      <ul className="d-flex flex-col gap-2 conversation-message-list" style={{ listStyle: "none", padding: 0, margin: 0 }}>
      ...
      <form className="conversation-text-form" onSubmit={handleTextSubmit}>
        <input
        ...
        />
        <button
          className="btn-primary conversation-send-button"
          type="submit"
      ...
      {textSendError && <p className="conversation-text-error" role="alert">{textSendError}</p>}
      <input
        ref={uploadInputRef}
        type="file"
