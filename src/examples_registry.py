...
def _service_entry_is_available(entry: dict) -> bool:
    ...
    if str(entry.get("provider") or "").strip().lower() in {"faster-whisper", "kokoro", "openrouter"}:
        return True
...
def _first_reachable_variant(variants: list[tuple[str, dict]]) -> tuple[str, dict] | None:
    for platform_name, entry in variants:
        if _service_entry_is_available(_rewrite_entry_for_host_runtime(entry)):
...
def _first_reachable_service_entry(section: dict) -> tuple[str, dict] | None:
    ...
    for key, entry in section.items():
        if isinstance(entry, dict) and _service_entry_is_available(entry):
...
def _resolve_service_default(example: EnrichedExample, category: str, service_id: str) -> ServiceDefault:
    ...
    if isinstance(local_entry, dict) and _service_entry_is_available(local_entry):
