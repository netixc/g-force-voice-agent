...
def _rewrite_endpoint_for_host_runtime(field: str, value: str) -> str:
    ...
    if field not in {"base_url", "server"} or value.strip().lower() == "in-process":
        return value
...
_PROFILE_MANAGED_SERVICE_PROVIDERS = frozenset({"faster-whisper", "kokoro", "openrouter"})
...
def _service_entry_is_available(entry: dict) -> bool:
    """Return whether a profile-managed provider is selected or its endpoint is reachable."""
    if str(entry.get("provider") or "").strip().lower() in _PROFILE_MANAGED_SERVICE_PROVIDERS:
        return True
    return is_endpoint_reachable(_entry_endpoint(entry))
...
def _filter_reachable_entries(catalog: dict) -> dict:
    ...
    for category, section in catalog.items():
        ...
        kept = {
            ...
            if not isinstance(entry, dict) or _service_entry_is_available(entry)
        }
...
def _first_reachable_variant(variants: list[tuple[str, dict]]) -> tuple[str, dict] | None:
    for platform_name, entry in variants:
        ...
        if isinstance(rewritten, dict) and _service_entry_is_available(rewritten):
