...
class FrontendBackendPipelineConfigTests(unittest.TestCase):
    def test_openrouter_connection_uses_separate_api_key_and_headers(self) -> None:
        with patch.dict(
            "os.environ",
            {
                "OPENROUTER_API_KEY": "openrouter-test-key",
                "OPENROUTER_HTTP_REFERER": "https://voice.example",
                "OPENROUTER_APP_NAME": "Voice Test",
            },
            clear=True,
        ):
            kwargs = frontend_backend_pipeline._llm_connection_kwargs("https://openrouter.ai/api/v1")

        self.assertEqual(kwargs["api_key"], "openrouter-test-key")
        self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
        self.assertEqual(kwargs["default_headers"]["HTTP-Referer"], "https://voice.example")
        self.assertEqual(kwargs["default_headers"]["X-OpenRouter-Title"], "Voice Test")

    def test_openrouter_connection_requires_api_key(self) -> None:
        with (
            patch.dict("os.environ", {}, clear=True),
            self.assertRaisesRegex(RuntimeError, "OPENROUTER_API_KEY"),
        ):
            frontend_backend_pipeline._llm_connection_kwargs("https://openrouter.ai/api/v1")

    def test_nvidia_connection_keeps_nvidia_api_key(self) -> None:
        with patch.dict("os.environ", {"NVIDIA_API_KEY": "nvidia-test-key"}, clear=True):
            kwargs = frontend_backend_pipeline._llm_connection_kwargs("https://integrate.api.nvidia.com/v1")

        self.assertEqual(
            kwargs,
            {"api_key": "nvidia-test-key", "base_url": "https://integrate.api.nvidia.com/v1"},
        )

    def test_kokoro_uses_configured_cuda_provider(self) -> None:
        with patch.dict("os.environ", {"ONNX_PROVIDER": "CUDAExecutionProvider"}, clear=True):
            provider = frontend_backend_pipeline._resolve_onnx_provider(
                ["CUDAExecutionProvider", "CPUExecutionProvider"]
            )

        self.assertEqual(provider, "CUDAExecutionProvider")

    def test_kokoro_rejects_unavailable_provider(self) -> None:
        with (
            patch.dict("os.environ", {"ONNX_PROVIDER": "CUDAExecutionProvider"}, clear=True),
            self.assertRaisesRegex(RuntimeError, "CUDAExecutionProvider.*unavailable"),
        ):
            frontend_backend_pipeline._resolve_onnx_provider(["CPUExecutionProvider"])
